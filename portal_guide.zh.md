...
3. 想换目标 — **拆掉告示牌**再重新放 `[Multi]`。
4. 目标暂时宕机时链接**不会变**；稍后再试或重建告示牌。
...
- 去**装有本插件**的服（配对 / 网络）— 物品是否跟随取决于 `/mvp items`。
...
从 [mp.mvse.ws](https://mp.mvse.ws/) 下载 jar — 文件名含版本号（`MultiversePortals-1.1.14.jar`）。
...
Transfer 地址自动取自 `server-ip` / `server-port`。仅当玩家用不同域名加入时才在 `config.yml` 设 `public-host`。检查：`/mvp settings`。
...
| 找不到在线服务器 | 再踩一次压力板 |
| 配对门损坏 | 用 `[Pair]` 代码重新配对 |
| 不在目录中 | `accept-transfers=true`、公网 IP/域名、非局域网；见 `/mvp settings` → catalog |
